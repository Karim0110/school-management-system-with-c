...
#include<iostream>
#include<fstream> // read stored info from another file

#include <sstream> // easy the update of string
#include <string> // string
#include <iterator> //for std::istream_iterator
#include <vector>
#include <iomanip> // manip and set width
using namespace std;
...
typedef struct fee //save to file
{
...
{
    fee s1;
    fstream outfile ("feeInfo.txt",ios::app); // read and info from stored files outfile open txt file
    cout<<"Class(Standard):";
    cin>>s1.classes;
    cout<<"Year:";
    cin>>s1.year;
...
void updateFee()
{
    ...
    fee s1;
    fstream infile ("feeInfo.txt"); // open files
    fstream tempfile ("tempInfo.txt",ios::app); // temporary files
    string line;
    while(std::getline(infile, line)) // search for spacific line 
    {
...
void listAllFees() // listing fees
{
    ifstream file("feeInfo.txt"); // open the whole folder
    string content;
    cout<<endl<<"\t\t\t\t !** FEES LIST **!\t\t\n\n\n";
    ...
    {
        istringstream ss(line); // read string as if in cin
        std::istream_iterator<std::string> begin(ss), end; //read succsisve eemnts from input
